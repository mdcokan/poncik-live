-- RETURN QUERY SELECT from private_room_signals was subject to RLS for the
-- invoker, yielding zero rows (empty RPC result) after a successful INSERT.
-- Return columns from the inserted row variable instead (same pattern as send_room_gift).
