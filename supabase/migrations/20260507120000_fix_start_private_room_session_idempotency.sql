...
begin
  ...
  set
    ...
    decided_at = now(),
    updated_at = now()
  where private_room_requests.room_id = v_request.room_id
    and private_room_requests.streamer_id = v_request.streamer_id
    and private_room_requests.id <> v_request.id
    and private_room_requests.status = 'pending';

  delete from public.room_presence
  where room_presence.room_id = v_request.room_id
    and room_presence.user_id not in (v_request.streamer_id, v_request.viewer_id);
